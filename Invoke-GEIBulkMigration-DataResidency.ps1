...
<#
.SYNOPSIS
    Bulk-migrates repositories from GitHub Enterprise Server to GitHub Enterprise Cloud with
...
.DESCRIPTION
    ...
    Use this script when the destination enterprise lives on SUBDOMAIN.ghe.com. Every GEI command
    is issued with --target-api-url so it reaches your residency region instead of api.github.com.
    For a standard github.com destination, use Invoke-GeiBulkMigration.ps1 instead.

    Required environment variables (never pass these as parameters):
      GH_PAT                            - Classic PAT for the TARGET org, created on
                                          https://SUBDOMAIN.ghe.com/settings/tokens (NOT github.com).
                                          Scopes: repo, admin:org, workflow (org owner)
                                                  repo, read:org, workflow (migrator role)
      GH_SOURCE_PAT                     - Classic PAT for the SOURCE org on GHES.
      ...
      AZURE_STORAGE_CONNECTION_STRING   - Azure Blob storage used to stage the migration archive.
                                          ...
                                          Not required when -UseGitHubStorage is specified.
      TARGET_API_URL                    - Default for -TargetApiUrl, e.g. https://api.acme.ghe.com
...
.PARAMETER GhesApiUrl
    API endpoint of the GitHub Enterprise Server instance.

.PARAMETER TargetApiUrl
    Base API URL for your enterprise's subdomain, e.g. https://api.acme.ghe.com.
...
.PARAMETER TargetRepoVisibility
    Enterprises with managed users (which includes all GHE.com enterprises) cannot host public
    repositories, so 'public' is not offered here.

.EXAMPLE
    .\Invoke-GeiBulkMigration-DataResidency.ps1 -RepoListPath .\repos.csv -TargetApiUrl https://api.acme.ghe.com -WhatIf

.EXAMPLE
    .\Invoke-GeiBulkMigration-DataResidency.ps1 -RepoListPath .\repos.csv -SkipExisting
#>
...
param(
    ...
    [string]$GhesApiUrl = 'https://github.com/api/v3',

    [ValidatePattern('^https://api\.[A-Za-z0-9-]+\.ghe\.com/?$')]
    [string]$TargetApiUrl = $env:TARGET_API_URL,

    [string]$DefaultTargetOrg = '#TARGET ORG#',
...
if (-not $TargetApiUrl) {
    throw 'TargetApiUrl is required. Pass -TargetApiUrl https://api.SUBDOMAIN.ghe.com or set $env:TARGET_API_URL.'
}
$TargetApiUrl = $TargetApiUrl.TrimEnd('/')

# Host that `gh api` talks to for target-side lookups, e.g. api.acme.ghe.com -> acme.ghe.com
$script:TargetHost = ([uri]$TargetApiUrl).Host -replace '^api\.', ''
...
function Assert-Prerequisites {
    ...
    if (-not $UseGitHubStorage) { $required += 'AZURE_STORAGE_CONNECTION_STRING' }
...
function Test-TargetRepoExists {
    ...
    # `gh api` reads GH_ENTERPRISE_TOKEN for any host other than github.com. Fall back to GH_PAT for
    # this call only, so a headless runner works and a local shell's environment is left alone.
    $previous = $env:GH_ENTERPRISE_TOKEN
    if (-not $previous) { $env:GH_ENTERPRISE_TOKEN = $env:GH_PAT }
    try {
        gh api --hostname $script:TargetHost "repos/$Org/$Repo" --silent 2>$null | Out-Null
        return ($LASTEXITCODE -eq 0)
    }
    finally {
        $env:GH_ENTERPRISE_TOKEN = $previous
    }
}
...
function Start-RepoMigration {
    ...
    if ($UseGitHubStorage) { $arguments += '--use-github-storage' }
    if ($NoSslVerify) { $arguments += '--no-ssl-verify' }
...
function Wait-RepoMigration {
    ...
    $output = & gh gei wait-for-migration --migration-id $MigrationId --target-api-url $TargetApiUrl 2>&1
...
Write-Host 'GEI bulk migration (GHE.com data residency)' -ForegroundColor Cyan
Write-Host "  Source (GHES) : $GhesApiUrl"
Write-Host "  Destination   : $TargetApiUrl"
Write-Host "  Target host   : $script:TargetHost"
Write-Host "  Storage       : $(if ($UseGitHubStorage) { 'GitHub-owned' } else { 'Azure Blob Storage' })"
...
if ($repos | Where-Object { $_.Status -eq 'Failed' }) { exit 1 }
